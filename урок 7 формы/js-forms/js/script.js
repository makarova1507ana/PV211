...
// это шаблон, который позволяет быстро найти необходимую полседовательность символов 

/* пример работы с регулярными выражениями 

поиск любого символа кроме данного - ^ [^3RUS]

отбор российских номеров автомобиля
[A-z]\d{3}[A-z]{2}\d{2,3}(RUS){0,1} 
(RUS){0,1} (RUS) - сгруппировали 
K100KK124RUS подходит
K100KK124 подходит
K100KK24RUS подходит
K100KK24 подходит

KRRRKK24  не подходит
K100KK144444424RUS не подходит
K100KKRRR24RUS не подходит
[A-z] или \w- поиск всех латинских букв в разных регистрах
\w - поиск всех латинских букв в разных регистрах + цифры
[А-я] -  поиск всех букв кирилицы в разных регистрах кроме буквы  ёЁ 
[А-яёЁ]- поиск всех букв кирилицы в разных регистрах с буквами ё Ё 
...
найти последовательность номеров российских(казахстан) телефона
(8|\+7)\d{10}  
(8|\+7) первый символ - 8 или первый символ + и второй-7
\d{10} ищет только 10 цифр подряд
89375484982 подходит
8937"484982 не подходит
+79375484982 подходит
893754849821111111  не подходит
89375 не подходит

ПОДУМАТЬ НАД ЭТИМИ ПРИМЕРАМИ ДОМА
8 937 548 49 82 подходит
8-937-548-49-82 подходит
...
\[\d{0,}\]  - ищет сноски
Кошка (лат. Felis catus) — домашнее животное, одно из наиболее популярных[1] (наряду с собакой) «животных-компаньонов»[2][3][4].
...
На протяжении 10 000 лет кошки ценятся человеком, в том числе за способность охотиться на грызунов и других домашних вредителей[10][11], а также за умение забавлять и утешать детей[12].
[123]
[1244444444444444444444444444444444444444444]
[12d]*/
...
//document.body.insertAdjacentHTML("beforeend", `<form name="some" class="some-form" action="#"><input type="text" name="nameInput" ></form>`);

// /*---------------------ПРАКТИКА ПРИМЕР С РЕГУЛЯРНЫМИ ВЫРАЖЕНИЯМИ----------------------- */

// input = document.getElementById('input_1');
// btn = document.querySelector('button');

// function isNumber() { //организуем проверку номера телефона  через рег выражения
//     if (input.value.match(/(8|\+7)\d{10}/g)) {// match если нашел сопадение создал массив со всеми найденными совпадениями
//         console.log('Есть номер телефона')
//         return true
//     }
//     console.log('Нет номера телефона')
//     return false
// }

// document.body.addEventListener('click', isNumber);
...
// }); // ожидаем когда элемент потеряет фокус
// input.addEventListener('blur', isNumber); // ожидаем когда элемент потеряет фокус
...
// 	mainFormInput.placeholder = "теперь можете писать";
// });
